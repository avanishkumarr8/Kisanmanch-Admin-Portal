...
import { getAllCancelledOrder } from '../../Services/OrderSevices';
...
const Cancelled = props => {
  ...
  const fetchData = async () => {
    try {
      const allCancelled = await getAllCancelledOrder()
      console.log(allCancelled)
      if (allCancelled) {
        setData(allCancelled);
        setSpin(true)
      }
    }
    catch (e) {
  ...
  const columns = [
    ...
    // {
    //   title: 'Product Name',
    //   dataIndex: ['products', "productsName"],
    //   key: 'products',
  ...
  return (
    ...
    <div style={{ background: 'white', paddingLeft: "20px", width: "98.5%", marginLeft: "14px", marginTop: "20px", marginBottom: "10px" }}>
      <Divider orientation="left" style={{ paddingLeft: "10px", paddingTop: 10 }}>Cancelled List</Divider>
      {spin
